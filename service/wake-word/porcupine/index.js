...
// Copied from https://raw.githubusercontent.com/Picovoice/porcupine/master/binding/javascript/porcupine.js and adapted
// slightly for node.js
...
module.exports = (function () {
    ...
    return async function (keywordModels, sensitivities) {
        const porcupineModule = await new Promise((resolve, reject) => {
            // PorcupineModule's Thenable implementation is very awkward, and will infinite loop if directly awaited...
            PorcupineModule().then((v) => {
                delete v.then;
                resolve(v);
            });
        });

        const initWasm = porcupineModule.cwrap('pv_porcupine_wasm_init', 'number', ['number', 'number', 'number', 'number']);
        const releaseWasm = porcupineModule.cwrap('pv_porcupine_wasm_delete', ['number']);
        const processWasm = porcupineModule.cwrap('pv_porcupine_wasm_process', 'number', ['number', 'number']);
        const sampleRate = porcupineModule.cwrap('pv_wasm_sample_rate', 'number', [])();
        const frameLength = porcupineModule.cwrap('pv_porcupine_wasm_frame_length', 'number', [])();
        const version = porcupineModule.cwrap('pv_porcupine_wasm_version', 'string', [])();
